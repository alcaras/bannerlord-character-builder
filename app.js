  ...
  if (isPlayer()) {
    ...
    ORIGIN.stages.forEach((st, i) => {
      ...
      for (const o of st.options) {
        ...
        // Use the game's own abbreviation (CTR, CNG), not the first three
        // letters of the id — that printed CON/CUN, which match no attribute
        // in the game and read as the wrong stat.
        const a = o.attr && ATTRS.find(x => x.id === o.attr);
        const extra = a ? ` — +1 ${a.abbrev}${sk ? ' · ' + sk : ''}`
          : (o.unspentAttr ? ` — +${o.unspentAttr} attr, +${o.unspentFocus} focus` : '');
        const opt = new Option(o.label + extra, o.id);
        if (a) opt.title = `+1 ${a.name}${sk ? ' · +1 focus and +10 levels: ' + sk : ''}`;
        sel.append(opt);
      }
...
function renderChosen() {
  ...
  if (byRole.size) {
    ...
    // distinctive template word and label the cluster.
    const STOP = new Set(['the', 'your', 'you', 'with', 'while', 'from', 'when',
      'for', 'and', 'are', 'per', 'each', 'more', 'less', 'not', 'due', 'that',
      'into', 'against', 'all', 'have', 'has']);
    const tokensOf = tpl => [...new Set(tpl.toLowerCase()
      .replace(/\{value\}/g, ' ').replace(/[^a-z\s-]/g, ' ').split(/\s+/)
      .map(w => w.replace(/s$/, '')).filter(w => w.length > 2 && !STOP.has(w)))];
    for (const role of roles) {
      const buckets = [...byRole.get(role).values()];
      const tokCount = new Map();
      for (const b of buckets)
        for (const t of tokensOf(b.tpl)) tokCount.set(t, (tokCount.get(t) || 0) + 1);
      const clusterSize = new Map();
      for (const b of buckets) {
        const shared = tokensOf(b.tpl).filter(t => tokCount.get(t) >= 2)
          .sort((x, y) => tokCount.get(y) - tokCount.get(x) || x.localeCompare(y));
        b.cluster = shared[0] || '';
        if (b.cluster) clusterSize.set(b.cluster, (clusterSize.get(b.cluster) || 0) + 1);
      }
      for (const b of buckets)
        if (b.cluster && clusterSize.get(b.cluster) < 2) b.cluster = '';
      buckets.sort((a, b) =>
        (a.cluster === '') - (b.cluster === '') ||
